  ...
  const fetchActiveData = async () => {
    try {
      ...
      const mergedOperators = allOperators.map((op) => {
        const readyData = assignReadyById.get(op.id);
        return {
          ...op,
          tractor: readyData?.tractor || op.tractor || null,
          isAssignReady: Boolean(readyData),
        };
      });
        ...
        <CardHeader className="bg-earth-card/50 border-b border-earth-dark/15/50 pb-4 sm:pb-5 pt-5 sm:pt-6 px-4 sm:px-6">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-sm sm:text-base text-earth-brown font-black tracking-wide uppercase">Assignment Queue</CardTitle>
              ...
              >
                ...
                {job.source === 'USSD' && !job.locationFixed && (
                  <div className="mb-4 p-3 bg-orange-500/5 border border-orange-500/10 rounded-xl">
                    <p className="text-[9px] font-bold text-orange-700 uppercase leading-relaxed tracking-wide">
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription className="text-[10px] sm:text-xs text-earth-sub font-bold mt-1">Available for assignment</CardDescription>
              </div>
            </div>
            <Badge className="bg-earth-primary/10 text-earth-green border border-emerald-500/20 font-black text-[9px] sm:text-[10px] uppercase tracking-widest px-3 py-1.5">{operators.filter(op => op.isAssignReady).length} Online</Badge>
          </div>
        </CardHeader>
          ...
          <div className={cn("transition-all duration-300", !selectedJob && "opacity-40 blur-[2px] pointer-events-none")}>
            ...
            <div className="space-y-3">
              {operators.map(op => {
                ...
                const canAssign = op.isAssignReady && selectedJob && selectedJob.status?.toUpperCase() !== 'PENDING';
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className={cn("text-[9px] font-black uppercase tracking-widest mt-1", op.isAssignReady ? "text-earth-green" : "text-earth-mut")}>
                          {op.isAssignReady ? "Ready for assignment" : "Unavailable / no ready tractor"}
                        </p>
                      </div>
                    </div>
                    <Button 
                      ...
                      disabled={!canAssign || isAssigning || (selectedJob?.source === 'USSD' && !selectedJob?.locationFixed)}
                      onClick={(e) => { 
                        e.stopPropagation(); 
                    ...
                    >
                      {!op.isAssignReady ? "Unavailable" : (selectedJob?.source === 'USSD' && !selectedJob?.locationFixed) ? "Job Locked" : isOperatorSelected && isAssigning ? "" : isOperatorSelected ? "Confirm" : "Assign"} {!isAssigning && <ArrowRight size={14} />}
                    </Button>
                  </Card>
                );
              })}
                  ...
                  <p className="text-[10px] font-bold text-earth-mut leading-relaxed uppercase">
                    {selectedJob?.source === 'USSD' && !selectedJob?.locationFixed
                      ...
                      : selectedJob?.status?.toUpperCase() === 'PENDING' 
                        ...
                        : "Select a scheduled job from the left queue to unlock the fleet for assignment."
                    }
                  </p>
               </div>
            </div>
